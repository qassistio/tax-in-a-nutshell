...
export default defineEventHandler(async (event) => {
  ...
  try {
    ...
    if (parsed.qualifier === 'acknowledgement') {
      ...
      await updateSubmission(row.id, { hmrc_raw_response: text })
    } else if (parsed.qualifier === 'response') {
      await updateSubmission(row.id, { hmrc_status: 'accepted', hmrc_raw_response: text, hmrc_message: 'Accepted by HMRC.' })
    ...
    await updateSubmission(row.id, { hmrc_message: `Last poll attempt failed: ${(err as Error).message}` })
  }
